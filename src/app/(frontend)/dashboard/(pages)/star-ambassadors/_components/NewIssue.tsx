...
import { getEligibility, newIssueFields, newIssueSchema } from './starFromData'
import { sendStarData } from '../_actions/sendStarCertificate'

export default function NewIssue() {

  // const [isEligible, setIsEligible] = useState<ReturnType<typeof getEligibility>>("not eligible")
  ...
  const onCheck = useCallback(
    (data: z.infer<typeof newIssueSchema>) => {
  ...
  const onApply = useCallback(
    async (data: z.infer<typeof newIssueSchema>) => {
      try {
        ...
        const result = await sendStarData(formData)
                ...
                {card.fields.map((fieldData) => {
                  return (
                    <FormField
                      ...
                      name={fieldData.name as keyof z.infer<typeof newIssueSchema>}
